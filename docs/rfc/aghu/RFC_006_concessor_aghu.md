...
- **Chamado por:** `ui_concessor.py` (mesma RFC, seção 12)
...
Essa separação permite que a política de perfis (o que pode ser automatizado) evolua no arquivo YAML sem alterar código, e garante que perfis críticos ou sujeitos a validação externa nunca cheguem à etapa de automação no navegador — eles são filtrados antes de qualquer interação com a tela.
...
| Módulo | Uso |
|---|---|
| `autenticador.py` (RFC-005) | `AGHU_URL` como default, `autenticar_aghu_page` para login sobre uma `Page` existente, `exigir_login_valido` para abortar em credencial inválida/timeout |
| `menu.py` (RFC-004) | `navegar_menu_aghu` para percorrer `CAMINHO_MENU_CADASTRO_USUARIO` e retornar o último iframe |
...
| `yaml` (opcional) | Leitura do catálogo de regras; se ausente, usa parser de fallback próprio (seção 6) |
...
| `STATUS_*` | strings (`concedido`, `ja_existente`, `bloqueado`, `validacao_ura`, `usuario_nao_encontrado`, `conferir_manual`, `erro`, `ignorado`) | Valores de `StatusPerfil`, usados como contrato com `ui_concessor.py` (seção 12) |
...
## 8. Preparação da Concessão (`preparar_concessao`)
...
## 10. Leitura da Planilha de Lote (`ler_planilha_concessoes`)
...
## 11. Execução: Preparação em Lote e Fluxo de Navegador

### 11.1 `_preparar_concessoes_lote`
...
### 11.2 Decisão de abrir o navegador
...
## 12. Navegação e Pesquisa de Usuário

### 12.1 `navegar_ate_cadastro_usuario`
...
### 12.2 `garantir_tela_pesquisa_usuario` — idempotência entre linhas do lote
...
### 12.3 `_pesquisar_usuario` e estados de resultado
...
## 13. Concessão de Perfis na Tela

### 13.1 Abrir a aba de perfis
...
### 13.2 Selecionar um perfil no autocomplete (`_selecionar_perfil_autocomplete`)
...
### 13.3 Preencher o protocolo (`_preencher_protocolo`)
...
### 13.4 Adicionar um perfil (`_adicionar_perfil`) e gravar em lote (`_conceder_perfis_na_tela`)
...
2. Para os demais, chama `_adicionar_perfil` (seleciona autocomplete → preenche protocolo → clica **Adicionar** → aguarda o perfil aparecer na tabela, `_aguardar_perfil_na_tabela`, até 10 s). Se o perfil não aparecer, lança `RuntimeError` incluindo qualquer mensagem do AGHUX capturada por `_mensagens_sistema`.
...
5. O resultado desse **único** `_gravar_perfis` (`sucesso` / `erro` / `indefinido`, via `_aguardar_mensagem_gravacao`, que procura a mensagem de sucesso `"perfil do usuario atualizado com sucesso"` ou termos de erro) é aplicado a **todos** os perfis do lote de adição — não há distinção por perfil individual nessa etapa.
...
## 14. Recuperação de Falhas Técnicas
...
Funções privadas (prefixo `_`) documentadas nas seções 7–15 não devem ser importadas por `ui_concessor.py` nem por outro módulo.

---

## 17. `ui_concessor.py` — Interface Gráfica
...
### 17.4 Proteção contra execução invisível
...
### 17.5 Bloqueio de controles durante execução
...
3. A gravação de perfis é feita em lote por usuário (um único **Gravar** por chamada a `_conceder_perfis_na_tela`), não por perfil — ver implicação na seção 13.4.
...
5. O parser de fallback do YAML (seção 7.2) é específico ao formato atual de `regras_perfis_aghu.yaml` e não deve ser tratado como parser YAML genérico.
6. A UI impede execução totalmente invisível (seção 17.4) e mantém Homologação como padrão operacional seguro.
7. O fluxo depende de textos/seletores visíveis do AGHUX: **Outros Módulos**, **Configuração**, **Acesso**, **Usuario**, **Pesquisar**, **Editar**/**Alterar**, **Adicionar**, **Gravar**, além dos IDs de campo listados na seção 5.
...
- Gravação em lote por usuário, com status definido diretamente pelo retorno de `_gravar_perfis` (sem reconferência ou retry pós-gravação).
...
- Contratos de status (`StatusPerfil`) entre núcleo e UI.
